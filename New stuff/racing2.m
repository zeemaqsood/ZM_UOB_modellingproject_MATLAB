...
% Test = [transpose(m:-1:1), 2 * ones(m, 1), transpose(1:m), [0;0.5;1], zeros(m, 1)];
    ...
    for j = 1:size(Test, 1)
        ...
        elseif new_Test(j, 2) == 4
            if Test(j, 1) == 40
                ...
                while k >= 0
                    [l, v] = find(new_Test(:, 1) == Test(j, 1) + dist(j) - k);
